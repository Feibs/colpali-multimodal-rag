#!/usr/bin/env python3
"""
ColPali RAG Multiple Retrieval Reranking Pipeline
        ...
        try:
            ...
            try:
                answer = self.query_vlm_api(query, 
                                            ...
                                            pages=results, 
                                            additional_query="\n\nEach page includes metadata such as its page number, version, and image of the page. The list is initially ordered by a similarity score, but I want you to independently evaluate the content of the provided pages (e.g., based on their text, layout, or visual cues) and version. Re-rank them based on their relevance and version. You should return the latest version, which is the higher version number, e.g. `v01` rather than `v00`. Ensure the reranking is based on the provided pages only, do not add other pages. If a page is irrelevant or unhelpful, feel free to exclude it from the result. \n\nReturn your output as a plain text in this format: {<page_number>: <final rank>} \n\nOnly return the plain text 'dictionary'. Do not include any explanation or extra text. If you think all the provided documents are not relevant to the query, return empty {}.")
